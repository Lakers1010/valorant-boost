...
const BOSS_PASSWORD = "boss1234"; // ← 可自行修改
  ...
  return (
    <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"16px 18px",marginBottom:10,position:"relative",overflow:"hidden"}}>
      ...
      <div style={{display:"flex",gap:8,marginTop:12,flexWrap:"wrap"}}>
        {agentName && order.status==="pending" && (
          <Btn onClick={()=>onClaim(order.id)} color="#4fc3f7" small>⚡ 接單</Btn>
        )}
        {agentName && order.status==="active" && order.claimedBy===agentName && (
          <Btn onClick={()=>onDone(order.id)} color="#69f0ae" small>✅ 完成</Btn>
        )}
        ...
        {!showBoss ? (
          ...
          <div style={{background:C.surface,border:`1.5px solid ${C.primary}`,borderRadius:16,padding:28}}>
            ...
            <input type="password" value={pw} onChange={e=>setPw(e.target.value)}
              ...
              style={{width:"100%",background:C.surface2,border:`1.5px solid ${pwErr?"#ef5350":C.border}`,borderRadius:10,padding:"12px 14px",color:C.text,fontSize:15,outline:"none",fontFamily:"'Noto Sans TC',sans-serif",marginBottom:pwErr?8:14,boxSizing:"border-box",transition:"border-color 0.2s"}} />
